...
async def start_banned(message: types.Message, state: FSMContext):
    await message.answer(
        lang.get("ukr").get("start_banned"), reply_markup=types.ReplyKeyboardRemove()
    )
...
async def login_start(message: types.Message, state: FSMContext):
    ...
    user = await find_in_db(
        collection_name="Model", data={"telegram_id": message.chat.id}
    )
    register_data = {}
    register_data.update(
        {
            ...
            "J_hair_color": get_hair_description(
                code=user.get("hair_code"), language=user.get("language")
            ),
        }
    )
    ...
    if not user.get("language"):
        ...
        await message.answer(
            l10.get("text").format(model_name=message.chat.full_name),
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
