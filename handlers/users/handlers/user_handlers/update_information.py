...
from handlers.users.handlers.agent_handlers.agent_events import (
    send_register_notification_to_agents,
    send_notification_to_agent,
)
from handlers.users.handlers.user_handlers.registration import (
...
async def finish_update_information(message: types.Message, state: FSMContext):
    ...
    if message.text == l10.get("registration_send_data").get("keyboard").get(
        "SendRegistrationSubmitButton"
    ):
        await state.reset_state(with_data=False)
        await message.bot.delete_message(
            message_id=data.get("message_to_delete"), chat_id=message.chat.id
        )
        ...
        if data.get("language") == "eng":
            ...
            full_name_eng = translit(
                register_data.get("B_full_name"), "uk", reversed=True
            )

        citizenship = pycountry.countries.get(
            alpha_2=register_data.get("C_citizenship").replace(":", "")
        )
        ...
        hair_code = get_hair_code(
            text=register_data.get("J_hair_color"), language=data.get("language")
        )
        ...
        await message.answer(
            l10.get("registration_finish").get("text"),
            reply_markup=types.ReplyKeyboardRemove(),
        )
        if is_user_registered:
            await update_to_db(
                collection_name="Model",
                parameter={"telegram_id": message.chat.id},
                data={
            ...
            if data.get("agent_telegram_id"):
                ...
                old_new = "\n".join(
                    [
                        f"{k[key]}': {deleted_data_stack[key]} -> {register_data[key]}"
                        for key in deleted_data_stack.keys() & register_data.keys()
                    ]
                )
                text_event = (
                    f"Модель змінила {message.chat.id} свої особисті дані:\n"
                    f"{old_new}"
                )

                await send_notification_to_agent(
                    agent_id=data.get("agent_telegram_id"), event=text_event
                )

            await login_start(message, state)
        else:
            await save_to_db(
                collection_name="InQueue",
                data={
            ...
            await send_register_notification_to_agents(
                model_telegram_id=message.chat.id,
            )
...
async def pop_from_register_state(call: types.CallbackQuery, state: FSMContext):
    ...
    if is_user_registered:
        ...
        user = await find_in_db(
            collection_name="Model", data={"telegram_id": call.message.chat.id}
        )
        register_data.update(
            {
                "A_phone_number": user.get("phone_number"),
                "B_full_name": user.get("full_name_eng")
                if data.get("language") == "eng"
                else user.get("full_name_ukr"),
                "C_citizenship": pycountry.countries.get(
                    name=user.get("citizenship")
                ).alpha_2,
                ...
                "J_hair_color": get_hair_description(
                    code=user.get("hair_code"), language=data.get("language")
                ),
            }
        )
    ...
    deleted_data_stack = (
        {} if not data.get("deleted_data_stack") else data.get("deleted_data_stack")
    )
    deleted_data_stack.update({call.data: register_data[call.data]})

    register_data.pop(call.data)
    await state.update_data(
        {"register_data": register_data, "deleted_data_stack": deleted_data_stack}
    )
