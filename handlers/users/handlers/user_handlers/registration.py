...
async def registration_handler(
    message: types.Message, state: FSMContext, is_information: bool = True
):
    await register_information(message, state, is_information)


async def register_information(
    message: types.Message, state: FSMContext, is_information=True
):
    ...
    for key in common_keys:
        ...
        if l10.get(key).get("keyboard") != {}:
            await message.answer(
                l10.get(key).get("text"),
                reply_markup=create_keyboard_from_dict(l10.get(key).get("keyboard")),
            )
        else:
            await message.answer(
                l10.get(key).get("text"), reply_markup=types.ReplyKeyboardRemove()
            )
        break
    else:
        await message.answer(
            ...
            .get("start_user")
            .get("register_user")
            .get("registration_send_data")
            .get("text"),
            reply_markup=create_keyboard_from_dict(
                lang.get(data.get("language"))
                .get("start_user")
                .get("register_user")
                .get("registration_send_data")
        ...
        message_to_delete = await message.answer(
            ...
            reply_markup=create_inline_keyboard_from_dict(
                lang.get(data.get("language"))
                .get("update_your_information")
                .get("keyboard")
            ),
