...
@dp.callback_query_handler(
    lambda c: c.data.startswith("register_model"), user_id=admins(), state=["*"]
)
async def register_model(call: types.CallbackQuery, state: FSMContext):
    model_telegram_id = int(call.data.split(":")[1])
    # data = await state.get_data()
    l10 = lang.get("agents").get("agent_events").get("register_model")
    await call.message.edit_text(l10.get("text"))
    model = await find_in_db(
        collection_name="InQueue", data={"telegram_id": model_telegram_id}
    )
    if model:
        model = dict(model)
...
async def send_notification_to_agent(agent_id, event):
    await dp.bot.send_message(chat_id=agent_id, text=event)
...
async def send_register_notification_to_agents(model_telegram_id):
    ...
    for admin in admins():
        ...
        button_text = l10.get("keyboard").get("RegisterModel")
        callback_data = f"register_model:{model_telegram_id}"
        keyboard.add(
        ...
        await dp.bot.send_message(
            chat_id=admin,
            text=l10.get("text").format(model_telegram_id=model_telegram_id),
